...
general as long as we have some base {@html _("b")}, we use the digits 0 to
{@html _("b-1")} to represent the number, with the obvious exception of 0.

<h3>Rational Bases</h3>
While we did say for a given base {@html _("b")} we use the digits 0 to {@html _(
  "b-1",
)}, we'll slightly modify this to say we'll use the digits 0 to {@html _(
  "|b|-1",
)} for a base {@html _("b")}, this would let us allow for negative bases with
...
i.e {@html _("\\frac1n")}, we can even flip around the traditional positive
...
<Accordion title="Base ½ example">
  ...
  Now to get the result the same process with base {@html _("\\frac1n")}. We can
  see easily all that needs to be done is to rotate everything around the
  decimal point

  <Bars top={[1, 0, 0, 1, ".", 1, 1, 1]} base={1 / 2} base_str={"½"}></Bars>

  Just as before, the way for base {@html _("b")} we used digits 0 to {@html _(
    "|b|-1",
  )}. For base {@html _("\\frac1b")} also we can do the same.
</Accordion>

Now what if we were to consider arbitrary rational bases rather than just those
which are of thr form {@html _("\\frac1n")}. So say how we write 265 in base {@html _(
  "\\frac73",
)}.
...
implies that we can get the number 265 in base {@html _("\\frac37")} also for free
by just rotating around the decimal point, i.e. 6.6346, which one can trivially verify.
<br />
<br />
This however also means that we will be using the same digits 0 to 6 for {@html _(
  "\\frac37",
...
The easiest to start with is an 'nth' root base, such as {@html _(
  "\\sqrt[4]{5}",
)}. We can see easily that this should be related to being in base 5. So
consider 42, in base 5, or {parseInt("42", 5)} in base 10. We can very easily first
write
...
should be trivial to see that {@html _("(\\sqrt[4]5)^{2n}")} are the standard place
values when we use 5 as our base.
<br />
<br />
So now if we were to create a new number 421 in base {@html _("\\sqrt[4]{5}")},
it's value would be {@html expand_str(
  [1, 2, 4, 2, 1],
  Math.pow(5, 0.25),
  "= 24.622 \\Leftrightarrow [6 + 2 \\times 5^{\\frac34} + 4 \\times 5^{\\frac24} + 2 \\times 5^{\\frac14}]",
  "\\sqrt[4]{5}",
)}. Or as an exercise you can verify that converting {@html _("\\sqrt{5}")} to ten
for 1234, is {@html _("[1234]_{\\sqrt5} = [14 + 8 \\sqrt5]_{10}")}. Finally we
can use these ideas to use the base {@html _("\\pi")}. Then 123 in base 3 would
be {@html expand_str([1, 2, 3], 3.1415926535, "\\approx 19.15", "\\pi")}.
...
the symbols of base 16, i.e, {@html _("[0 \\rightarrow 9, A \\rightarrow F]")},
but in base 7. Then the number F1 would be {@html expand_str(
  [15, 1],
  7,
  "= 106_{10}",
...
a cube root of unity, {@html _("\\omega = \\frac{-1}2 + \\frac{i\\sqrt3}2")}. So
a simple number like 1234 would be
